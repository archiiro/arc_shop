...
export class PopulationGroupComponent implements OnInit {
  ...
  reloadTable() {
    this.isLoading = true;
    this.service.pagingPopulationGroups(this.searchObject)
      .subscribe({
        next: (response) => {
  ...
  update(populationGroup: PopulationGroup) {
    this.service.getPopulationGroups(populationGroup.id).subscribe({
      next: (response) => {
  ...
  delete(id: string) {
    this.service.deletePopulationGroups(id)
      .subscribe({
        next: () => {
          this.reloadTable();
