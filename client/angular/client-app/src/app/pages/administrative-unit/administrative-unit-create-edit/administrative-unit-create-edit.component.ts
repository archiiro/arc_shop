        ...
        this.service.saveOrUpdateAdministrativeUnitByParentId(this.objForm, this.parentId)
          .subscribe({
            next: (response) => {
              ...
              if (this.level == 5) {
                this.router.navigate(["setting/location/getCommune/" + this.parentId]);
              } else if (this.level == 4) {
                this.router.navigate(["setting/location/getDistrict/" + this.parentId]);
              }
            },
            error: (error) => {
              console.log(error)
            ...
            next: (response) => {
              ...
              if (this.isCreateMode) {
                ...
                this.service.getAdministrativeUnit(this.id).subscribe({
                  next: (res) => {
                    if (res.level == 5 ) {
                      this.router.navigate(["setting/location/getCommune/" + res.parentDto.id]);
                    } else  if (res.level == 4 ) {
                      this.router.navigate(["setting/location/getDistrict/" + res.parentDto.id]);
                    } else {
                      this.router.navigate(["setting/location"]);
                    }
                  },
