...
export class AdministrativeUnitComponent implements OnInit, AfterViewInit {
  ...
  titlePage: string = "Danh sách Tỉnh/Thành phố";
  titleLevel: number = 0;
  ...
  columns = [
    ...
    { name: 'Thứ tự', prop: 'position', visible: true },
    {name : 'PQM', prop:'pqmCode', visible: true},
    { name: 'HTC Elog', prop: 'elogCode', visible: true },
    { name: 'HIV Info', prop: 'hivInfoCode', visible: true }
  ];
  ...
  handleGetChildren(row: AdministrativeUnit){
    ...
    if(this.level == 0){
      this.router.navigate(["setting/location/getDistrict/" + row.id]);
    }else{
      this.router.navigate(["setting/location/getCommune/" + row.id]);
    }
    this.reloadTable();
  }

  handleEdit(id: string){
    this.router.navigate(["setting/location/edit/" + id]);
  }
  handleCreate(){
    this.router.navigate(["setting/location/create/" + (this.id ? this.id : "null")]);
  }
