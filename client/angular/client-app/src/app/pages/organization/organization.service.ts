...
export class OrganizationService {
  ...
  private readonly END_POINT = this.configInitService.apiBaseUrl + '/api/v1/organizationext';
  ...
  getAllOrganization(pageIndex: number, pageSize: number): Observable<any> {
    const url = this.END_POINT + "/"+ pageIndex + "/" + pageSize
  ...
  getAllParentOrganization(searchObject: SearchObject): Observable<any> {
    var requestHeaders = new HttpHeaders();
